...
1. **硬件连接**：
   ...
   - 按照 `cst816s.py` 中的配置连接 CST816S 触摸屏（I2C: SCL=GPIO6, SDA=GPIO7, RST=GPIO5）。

2. **WiFi 配置**：
   - 修改 `watch_touch.py` 中的 `SSID` 和 `PASSWORD` 为您的 WiFi 名称和密码。
...
4. **上传代码**：
   - 将所有 Python 文件（`display_driver.py`、`cst816s.py`、`watch_touch.py`、`task_handler.py`、`fs_driver.py` 等）上传到 MicroPython 设备。
...
2. **屏幕切换**：
   - **右滑手势**：切换到下一个屏幕（时间 → 天气 → 日期 → 二维码 → 时间）。
   - **左滑手势**：切换到上一个屏幕（时间 → 二维码 → 日期 → 天气 → 时间）。
